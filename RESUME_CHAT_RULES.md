...
8. Persist a v2 Revision 7 package config at `scripts/packages/<slug>.json` using `scripts/examples/package-v2.json` as the contract. Use the self-contained showcase pattern by default: `routeMode: "scoped-projects"`, `route.presentation: "showcase"`, `route.heroIntent: "resume-support"`, and `route.showcaseSections: ["chapters", "capabilities"]`. Persist `route.designConcept` as `editorial-proof` or `proof-grid`; honor the human's named concept and default to `editorial-proof` without pausing when none is named. Editorial Proof publishes to `wallymo/wallymo.github.io` at `https://wallymo.github.io/`; Proof Grid publishes to `wally-mostafa/wally-mostafa.github.io` at `https://wally-mostafa.github.io/`. Use `full` or `canonical-projects` only as an explicit, intentional opt-out when the public portfolio narrative is part of the role's proof. Write a concise summary that fits the page-one layout, choose four to six supported skills, preserve evidence attribution and role metadata, and use unique `addition:<slug>` IDs only for new supported bullets.
...
16. Treat the role-specific `Portfolio` annotation as publish-blocking. Commit the built source package. If its registered publish repository is a separate checkout, stage only its required public files with `node scripts/stage-tailored-package.mjs --config scripts/packages/<slug>.json --target-root <absolute-checkout-path>`, then commit and push that target checkout before live verification.
...
18. Run `node scripts/verify-tailored-route.mjs <route-slug>` from the workflow source checkout after publishing to the concept's registered repository. It must resolve the concept-specific public root, verify live `200` responses and local/live config, route, scoped-project, and PDF checksum parity, and confirm the publish-repository metadata before changing `publishStatus` to `live-verified`. Commit and push the resulting verification-only manifest update in the workflow source repository.
...
- Treat **Editorial Proof** as the control concept and **Proof Grid** as the challenger. `proof-grid` requires scoped project pages and a package-local snapshot of its concept CSS. If the same JD is tested in both concepts, create distinct slugs, artifact stems, routes, and PDFs so each resume retains exactly one Portfolio destination.
...
- Use `scoped-projects` with `route.presentation: "showcase"`, `route.heroIntent: "resume-support"`, and `route.showcaseSections: ["chapters", "capabilities"]` by default. The route, selected case studies, and all return paths must stay inside the employer-specific package. If the human explicitly asks to restore How I Build or the career arc without the full homepage narrative, append only the requested section IDs to `route.showcaseSections`.
