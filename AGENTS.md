...
- For every new or intentionally rebuilt tailored route, select 4 to 6 role-relevant projects and order them by relevance rather than default chronology. Include `project-03.html` — Enterprise Data Visualization — as the signature fourth project unless that position would weaken the JD-specific sequence. When `project-07.html` — Other Highlights — is selected, treat it as the closing reel. Keep an existing verified package unchanged unless it is intentionally rebuilt. Use only 3 projects when the human explicitly asks for a compact three-project route.
