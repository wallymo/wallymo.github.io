...
- Account-management packages get the switchboard hero. The homepage hero ships the ask-to-proof machine (`assets/portfolio-revision/hero-machine.js`); when a package's `classification.targetLane` is `client-account-delivery` or its `fitGate.resumeBase.mode` is `account-leadership`, the builder swaps in the switchboard (`assets/portfolio-revision/hero-switchboard.js`) in the same hero slot. When the human says "use the account mgmt portfolio" (or equivalent) for a JD that does not already resolve that way, set `route.heroAnimation: "switchboard"`; set `route.heroAnimation: "machine"` to keep the machine on an account package. Only the hero scene, its accessible label, and its caption change; the hero copy, layout, and controls stay the homepage's.
- Every new or rebuilt switchboard package must set `route.heroBoard`, which decides the switchboard's lines and calls. Use `"pharma"` when the JD is pharma, healthcare, or life sciences. For any other account JD, tailor a board in the JD's own vocabulary: an object with exactly 4 `client` line labels (max 16 characters each), 6 `teams` labels (max 18), and 4 `calls`, each `{ "from": <client label>, "to": [1-2 team labels], "ask": <max 36 chars>, "done": <max 28 chars> }`. Every client line makes exactly one call and every team is patched at least once. The calls are short, plain illustrations of that role's day-to-day work, never claims about specific results. Board copy goes through the humanizer and claim gates. Use `"general"` only when the JD is too thin to tailor well. The human can override with "pharma board" or "general board".
- Every new or intentionally rebuilt package must persist `route.designConcept`. Accept the human-facing choices **Editorial Proof** (`editorial-proof`) and **Proof Grid** (`proof-grid`). If the human names a concept, use it. If no concept is named, default to `editorial-proof` and continue the authorized workflow without adding a confirmation stop. `proof-grid` requires `routeMode: "scoped-projects"` so the route and every case study stay in the same visual system.
- Publish each package to the repository and public root registered for its concept: Editorial Proof uses `wallymo/wallymo.github.io` at `https://wallymo.github.io/`; Proof Grid uses `wally-mostafa/wally-mostafa.github.io` at `https://wally-mostafa.github.io/`. The generated resume, cover letter, Open Graph metadata, manifest, checker, and verifier must all use that same concept-specific root.
- Keep `wallymo/wallymo.github.io` as the workflow source of truth. When a package's registered publish repository is a different checkout, run `node scripts/stage-tailored-package.mjs --config scripts/packages/<slug>.json --target-root <absolute-checkout-path>` after the source package passes locally. Use `--overwrite` only after reviewing an intentional rebuild. The stager must reject a mismatched remote, unrelated dirty target files, missing artifacts, or unreviewed target differences. Commit and push the staged public artifacts in the registered publish repository; never copy the whole source workspace.
- A tailored resume still carries one `Portfolio` link to its matching role route. For a same-JD A/B test, build two separately named packages, routes, and PDFs so each artifact has one unambiguous concept and destination; do not point one PDF at two portfolio URLs.
...
- Use `scripts/resume-foundation.json`, imported from the human-supplied `Wally-Mostafa-Resume.pdf`, as the shared evidence catalog and `scripts/resume-base-profiles.json` as the versioned registry for the approved Account Leadership and AI/Product evidence pools. Preserve source attribution, employers, titles, dates, awards, education, and truthful evidence boundaries. Write a concise summary from a compatible positioning lane and select four to six evidence-backed skills. Do not enforce a summary word count; enforce page-one placement, readable type, and the two-page output contract instead.
- Treat the resume summary as orientation, not a proof section. Explain the professional identity, career arc, relevant domains, and overall way of working in summary form. Do not stack metrics, named achievements, or supporting evidence that the experience section already proves.
- Revision 7 requires `fitGate.resumeBase` before tailoring. Select `account-leadership` when relationships, account planning, budgets, delivery, retention, or adoption are the primary operating center. Select `ai-product-implementation` when AI delivery, product ownership, workflow design, UX, research, or implementation are primary. Select `hybrid-selective` only when direct core requirements map to both profiles; generic client or technology language is not enough. Industry never decides the base.
- Every employer JD uses `resumeBase.action: tailor-to-jd`. `use-existing` is limited to general networking without an employer JD. Current healthcare and AI resumes remain the two general networking artifacts; there is no standalone neutral resume, and hybrid artifacts exist only for qualifying JDs.
- For hybrid mode, `leadProfileId` controls summary emphasis and section order without limiting truthful selection from either evidence pool. Hybrid means selective composition, not concatenating two full resumes. Use `accountPresentation: agency-progression` when account tenure, seniority, or title progression is a central screen; use `consolidated` when account experience supports an AI/product-led role without serving as the primary tenure screen.
...
- For every new or intentionally rebuilt tailored route, pick 3 role-relevant projects and order them by relevance rather than default chronology, then include `project-03.html` — Enterprise Data Visualization — as the signature fourth project. If Enterprise Data Visualization is already one of the 3 strongest role matches, move it to fourth and replace its earlier slot with the next-best role-relevant project. When `project-07.html` — Other Highlights — is selected, treat it as the closing reel: use 2 role-relevant projects first, place Enterprise Data Visualization third, and place Other Highlights fourth. Keep an existing verified package unchanged unless it is intentionally rebuilt. Use only 3 projects when the human explicitly asks for a compact three-project route, and use 5 only when the human explicitly asks for a deeper version.
...
- New JD showcase configurations must explicitly include `route.showcaseSections: ["chapters", "capabilities"]`. Keep the three career chapters directly after selected work and keep Capabilities on every route. Include both sections' inherited visible and accessible copy in the humanizer and claim review. Existing packages keep their published state until intentionally rebuilt; every rebuild must adopt both required sections.
...
- Do not rewrite capabilities, trust strip, awards, experience arc, visual system, or other non-project sections unless the human explicitly asks for a broader portfolio edit. Capabilities stays inherited and visible on every route; the role-specific closing contact/footer copy is the only default exception.
- If the human explicitly asks to restore How I Build or the career arc on a showcase route, use `route.showcaseSections` to add only the requested canonical section instead of switching the whole route to `full` presentation. Chapters and Capabilities remain required.
- For every new or intentionally rebuilt package, persist the complete workflow v2 Revision 7 config under `scripts/packages/<slug>.json`. Record the resume-base decision, positioning, requirement source/confidence/proof/destinations, match mode, and conditional cover-letter contract. A supported core requirement must have resume evidence; a cover letter cannot be its only destination. Ambiguous JD language cannot independently fail a hard gate or produce `not-fit` until the employer or application form confirms it. Existing Revision 6 packages remain valid and are not migrated automatically.
...
- After the source package is committed and its staged artifacts are pushed to the concept's registered publish repository, run `node scripts/verify-tailored-route.mjs <slug>` from the workflow source checkout. A Revision 7 package is not `live-verified` until the resume-base registry, config, route, projects, resume PDF, and any required cover-letter artifacts return `200` from the concept-specific public root and their live checksums match the QA-approved local files. Commit and push the resulting verification-only manifest update in the workflow source repository; the deployment repository contains the public artifact mirror, not the source manifest history.
...
- After the fit class, make the Revision 7 resume-base decision from the JD's primary operating center. Report `account-leadership`, `ai-product-implementation`, or a justified `hybrid-selective` choice, plus the lead profile, account presentation, action, and rationale. Do not infer hybrid from incidental language or choose a base from the employer's industry.
