...
- Use `scripts/resume-foundation.json`, imported from the human-supplied `Wally-Mostafa-Resume.pdf`, as the shared evidence catalog and `scripts/resume-base-profiles.json` as the versioned registry for the approved Account Leadership and AI/Product evidence pools. Preserve source attribution, employers, titles, dates, awards, education, and truthful evidence boundaries. Write a 45–65-word summary from a compatible positioning lane and select four to six evidence-backed skills.
