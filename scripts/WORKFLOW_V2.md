...
An explicit failed hard gate may remain in a user-approved `stretch` package. This exception requires `--allow-stretch`, `positioning.applicationStrategy: "approved-stretch"`, a `not-credible` cover-letter bridge, and `coverLetter: null`. The config must keep the failed requirement and its gap visible; unresolved gates and `not-fit` packages still stop generation. This permits an honest stretch package, not a claim that the requirement is met or an application-readiness finding.
