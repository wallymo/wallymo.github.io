...
The homepage hero ships the ask-to-proof machine. Account-management routes use the switchboard instead: `route.heroAnimation` accepts `machine` or `switchboard`, and when it is omitted the builder chooses `switchboard` for the `client-account-delivery` lane or an `account-leadership` resume base, and `machine` otherwise. The swap replaces only the hero script, the canvas's accessible label, and the figure caption, so the hero copy, layout, pause control, and reduced-motion behavior stay the homepage's. Staging picks up `assets/portfolio-revision/hero-switchboard.js` through the route's dependency closure. The builder fails if a switchboard route is built from an `index.html` that no longer carries the machine hero markup.

The switchboard's lines and calls come from `route.heroBoard`, which every new or rebuilt switchboard package must set. `"pharma"` (Brand team, Vendors, Med/Legal, Sales team; Creative, UX/UI, Production, Media, Digital, Finance) serves pharma, healthcare, and life-sciences JDs. Other account JDs get a board tailored to the JD: an object with exactly 4 `client` labels (16 characters max), 6 `teams` labels (18 max), and 4 `calls` of `{ from, to, ask, done }` where `from` is a client label, `to` lists one or two team labels, `ask` is at most 36 characters, and `done` at most 28. Every client line makes exactly one call and every team is patched at least once, so the frame fills evenly and nothing overflows the board or the caller's bubble. Tailored copy is illustrative day-to-day work in the JD's vocabulary, not a claim, and it enters the humanizer and claim gates as `route.heroBoard.*`. `"general"` (Client lead, Exec sponsor, End users, Procurement; Product, Design, Engineering, Analytics, Support, Finance) is the fallback for JDs too thin to tailor. The builder stamps the choice on the hero stage as `data-board`, with a tailored board embedded as JSON; the hero script falls back to the general board if that JSON is missing or malformed.

Every new or rebuilt package also sets `route.designConcept`. Editorial Proof (`editorial-proof`) publishes to `wallymo/wallymo.github.io` at `https://wallymo.github.io/`. Proof Grid (`proof-grid`) requires scoped project pages and publishes to `wally-mostafa/wally-mostafa.github.io` at `https://wally-mostafa.github.io/`. The concept registry is authoritative for the public root and repository; generated route metadata, resume and cover-letter links, manifest records, package checks, application readiness, and live verification must resolve the same target. Historical configs without the field continue to resolve to Editorial Proof.
...
- Reviewed authored copy: inherited content from explicitly selected showcase sections, fit classification explanations and evidence, requirement evidence, fit-gate rationale, resume copy, route hero copy, contact copy, and the full cover letter when present.
