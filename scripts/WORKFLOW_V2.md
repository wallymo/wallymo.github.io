...
A package may set `route.presentation` to `showcase` when the role route should read as a quiet exhibit of relevant work rather than a positioning pitch. Packages in the `client-account-delivery` lane default to showcase; set `route.presentation` to `full` explicitly to opt out. Showcase mode drops the homepage's How I Build, capabilities, and career-arc sections (and their nav links) and the "Also from this period" highlights strip from the generated route, keeping the tailored hero, trust logos, work grid, awards, and contact. Pair showcase mode with `routeMode: "scoped-projects"` so case-study pages keep the visitor inside the curated route instead of linking back to the canonical homepage. Optional `route.workHeading` and `route.contactHeading` replace the work-section and contact-section headings; both are authored copy and go through the humanizer and claim gates. Showcase mode reduces the contact section to its heading and links, omitting the section label and the `contact.prompt` subtitle; the prompt stays required because full-presentation routes and the copy gates still use it. Omitting `route` keeps the full homepage presentation.
