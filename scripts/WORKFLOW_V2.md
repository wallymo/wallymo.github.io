...
A package may set `route.presentation` to `showcase` when the role route should read as a quiet exhibit of relevant work rather than a positioning pitch. Showcase mode drops the homepage's How I Build, capabilities, and career-arc sections (and their nav links) from the generated route, keeping the tailored hero, trust logos, work grid, awards, and contact. An optional `route.workHeading` replaces the work-section heading; it is authored copy and goes through the humanizer and claim gates. Omitting `route` keeps the full homepage presentation.
